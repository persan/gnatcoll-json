...
package body GNATCOLL.JSON.Support.Ada.Containers.Indefinite_Ordered_Maps_SImple.Generic_Tests is
   ...
   begin
      Test.Test_Data :=  new Map'(Initialize);
   end Set_Up_Case;
   ----------------
   -- Test_Write --
   ----------------
   ...
   procedure Test_Read (Test : in out AUnit.Test_Cases.Test_Case'Class)  is
      Td     : Test_Case renames Test_Case (Test);
      Result : constant Map := Get (GNATCOLL.JSON. Read (Read (Ada2file (Unit_Name)), Filename => Ada2file (Unit_Name)));
   begin
...
end GNATCOLL.JSON.Support.Ada.Containers.Indefinite_Ordered_Maps_SImple.Generic_Tests;
