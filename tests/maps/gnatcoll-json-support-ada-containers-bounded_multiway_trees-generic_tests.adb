...
package body GNATCOLL.JSON.Support.Ada.Containers.Bounded_Multiway_Trees.Generic_Tests is
   ...
   begin
      Test.Test_Data := new Tree'(Initialize);
   end Set_Up_Case;
   ----------------
   -- Test_Write --
   ----------------
   ...
   procedure Test_Read (Test : in out AUnit.Test_Cases.Test_Case'Class)  is
      Td     : Test_Case renames Test_Case (Test);
      Result : constant Tree := Get (GNATCOLL.JSON.Read (Read (Ada2file (Unit_Name)), Filename => Ada2file (Unit_Name)));
   begin
   ...
   begin
      ...
      declare
         Result : constant Tree := Get(J,"testData");
      begin
         Assert (Result = Td.Test_Data.all, "data mismatch");
      end;
