...
package body GNATCOLL.JSON.Support.Test.Ada.Containers.Indefinite_Vectors is
   ...
   begin
      ...
      String'Write (Text_Streams.Stream (F), GNATCOLL.JSON.Write (Create (Td.Test_Data.all), Compact => False));
      Close (F);
   end Test_Write;
   ...
   begin
      ...
      begin
         ...
         Td.Result := new V.Vector'(Get (GNATCOLL.JSON. Read (Buffer, Filename => DataPath)));
      end;
      Close (F);
