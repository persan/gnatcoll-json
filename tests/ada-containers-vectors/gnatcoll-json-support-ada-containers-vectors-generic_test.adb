...
package body GNATCOLL.JSON.Support.Ada.Containers.Vectors.Generic_Test is
   ...
   begin
      Write (Ada2file (Unit_Name), GNATCOLL.JSON.Write (Create (Create (Td.Test_Data)), Compact => False));
   end Test_Write;
   ...
   procedure Test_Read (Test : in out AUnit.Test_Cases.Test_Case'Class)  is
      Td   : Test_Case renames Test_Case (Test);
   begin
      Td.Result := Get (GNATCOLL.JSON. Read (Read (Ada2file (Unit_Name)),
   ...
   begin
      Set_Field (D, "D1", Td.Test_Data);
      declare
         Res : constant V.Vector := Get (D, "D1");
      begin
         Assert (Res = Td.Test_Data, "data mismatch");
      end;
   end Test_Get_Set;
...
end GNATCOLL.JSON.Support.Ada.Containers.Vectors.Generic_Test;
